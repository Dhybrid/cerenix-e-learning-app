...
Future<void> updateProfile({
  ...
  try {
    ...
    if (location != null && location.isNotEmpty) requestData['location'] = location;
    ...
    if (response.statusCode == 200) {
      ...
      try {
        final errorData = jsonDecode(response.body);
        final error = errorData['error'] ?? 'Failed to update profile (Status: ${response.statusCode})';
        throw Exception(error);
      } catch (e) {
...
Future<List<BillingPlan>> getBillingPlans(String universityId) async {
  try {
    print('📋 [ApiService] Getting billing plans for university: $universityId');
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData is List) {
        print('✅ [ApiService] Billing plans loaded successfully: ${responseData.length} plans');
        ...
        // Try to parse plans
        try {
          final plans = responseData.map((json) => BillingPlan.fromJson(json)).toList();
          print('✅ Successfully parsed ${plans.length} billing plans');
          return plans;
        } catch (e) {
          print('❌ ERROR parsing billing plans: $e');
          print('🔍 This means the JSON structure doesnt match BillingPlan model expectations');
          return [];
        }
      } else {
        print('❌ [ApiService] Expected List but got: ${responseData.runtimeType}');
      ...
      print('❌ [ApiService] API Error: $error (Status: ${response.statusCode})');
      throw Exception('$error (Status: ${response.statusCode})');
    }
  } catch (e) {
...
Future<List<Course>> getCoursesForUser() async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final error = errorData['error'] ?? 'Failed to load courses (Status: ${response.statusCode})';
      throw Exception(error);
    }
  } on TimeoutException {
    ...
    for (var course in courses) {
      try {
        ...
        if (topics.isNotEmpty) {
          ...
          print('   - ${course.code}: $progress% ($completedCount/$totalTopics topics)');
...
Future<List<CourseOutline>> getCourseOutlines(int courseId) async {
  try {
    ...
    final response = await http.get(
      Uri.parse('${ApiEndpoints.baseUrl}/api/academics/course-outlines/?course=$courseId'),
      headers: {'Content-Type': 'application/json'},
    ...
    if (response.statusCode == 200) {
      ...
      final outlines = data.map((json) => CourseOutline.fromJson(json)).toList();
...
Future<List<Topic>> getTopics({int? courseId, int? outlineId}) async {
  try {
    ...
    if (response.statusCode >= 400) {
      ...
      try {
        final errorData = json.decode(response.body);
        final error = errorData['detail'] ?? errorData['error'] ?? 'Failed to load topics';
        throw Exception('$error (Status: ${response.statusCode})');
      } catch (_) {
    ...
    if (responseData is List) {
      ...
      else if (responseData.containsKey('topics') && responseData['topics'] is List) {
    ...
    print('📊 Successfully parsed ${topics.length} out of ${topicsData.length} items');
    
    // Cache topics for offline use
    if (topics.isNotEmpty) {
      final cacheKey = outlineId != null ? 'outline_$outlineId' : 'course_$courseId';
...
Future<Map<String, dynamic>> submitTopicAnswer({
  ...
  try {
    ...
    if (selectedAnswer != null && selectedAnswer.isNotEmpty) {
      ...
      throw Exception('Either selected_answer or answer_text must be provided');
    }

    print('📤 Sending answer data: $requestData');
    
    final response = await http.post(
      Uri.parse('${ApiEndpoints.baseUrl}/api/content/topics/$topicId/submit_completion/'),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = json.decode(response.body);
      final error = errorData['detail'] ?? errorData['error'] ?? 'Failed to submit answer';
      throw Exception('$error (Status: ${response.statusCode})');
    }
  } on TimeoutException {
...
Future<Map<String, dynamic>> updateTopicProgress({
  ...
  try {
    ...
    final response = await http.post(
      Uri.parse('${ApiEndpoints.baseUrl}/api/content/topics/$topicId/update_progress/'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode(requestData),
    ).timeout(const Duration(seconds: 10));
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final errorData = json.decode(response.body);
      final error = errorData['detail'] ?? errorData['error'] ?? 'Failed to update progress';
      throw Exception('$error (Status: ${response.statusCode})');
    }
  } on TimeoutException {
    print('⏰ Request timeout');
...
Future<Map<String, dynamic>> submitTopicCompletion({
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['is_correct'] == true && responseData['topic_completed'] == true) {
      ...
      final errorData = json.decode(response.body);
      final error = errorData['detail'] ?? errorData['error'] ?? 'Failed to submit completion';
      throw Exception('$error (Status: ${response.statusCode})');
    }
  } on TimeoutException {
    print('⏰ Request timeout');
...
Future<void> _updateLocalTopicProgress(int topicId, int progressPercentage, bool isCompleted) async {
  try {
    ...
    print('✅ Progress saved locally for topic $topicId: $progressPercentage%');
...
Future<void> _cacheCourseOutlines(int courseId, List<CourseOutline> outlines) async {
  try {
...
Future<List<CourseOutline>> _getCachedCourseOutlines(int courseId) async {
  try {
    ...
    if (cachedData != null && cachedData is List) {
      final outlines = cachedData.map((json) => CourseOutline.fromJson(json)).toList();
      print('📂 Loaded ${outlines.length} outlines from cache for course $courseId');
      return outlines;
    }
  } catch (e) {
...
Future<List<Topic>> _getCachedTopics(String cacheKey) async {
  try {
    ...
    if (cachedData != null && cachedData is List) {
      ...
      print('📂 Loaded ${topics.length} topics from cache with key: $cacheKey');
      return topics;
    }
  } catch (e) {
...
Future<CourseProgressSummary> getCourseProgressSummary(int courseId) async {
  try {
    ...
    print('✅ Course progress summary: ${summary.progressPercentage}% complete');
    return summary;
  } catch (e) {
...
Future<List<PastQuestionSession>> getPastQuestionSessions() async {
  try {
    ...
    final url = '${ApiEndpoints.baseUrl}/api/content/academic-sessions/all_sessions/';
    final response = await http.get(
      Uri.parse(url),
      headers: {'Content-Type': 'application/json'},
    ).timeout(const Duration(seconds: 10));
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData is List) {
        ...
        print('📥 Paginated response: ${responseData['count']} total sessions');
      } else {
        print('❌ Unexpected response format: ${responseData.runtimeType}');
        return [];
      }
...
Future<List<PastQuestion>> getPastQuestions({
  ...
  try {
    ...
    final uri = Uri.parse('${ApiEndpoints.baseUrl}/api/content/past-questions/');
    final params = <String, String>{
      'course': courseId,
    };
...
Future<List<PastQuestionSession>> getSessionsForCourse(String courseId) async {
  try {
    print('📅 Getting sessions for course: $courseId');
    
    final url = '${ApiEndpoints.baseUrl}/api/content/past-questions/sessions/?course=$courseId';
...
Future<Map<String, dynamic>> submitPastQuestionAnswer({
  ...
  try {
    ...
    final response = await http.post(
      Uri.parse('${ApiEndpoints.baseUrl}/api/content/past-questions/$questionId/submit_answer/'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode(requestData),
    ).timeout(const Duration(seconds: 10));
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      return {
        ...
        'correct_answer': responseData['correct_answer'],
        'explanation': responseData['explanation'] ?? responseData['solution_text'],
...
Future<Map<String, List<PastQuestion>>> getPastQuestionsByTopic({
  ...
  try {
    ...
    final uri = Uri.parse('${ApiEndpoints.baseUrl}/api/content/past-questions/by_topic/');
    final params = <String, String>{
      'course': courseId,
    };
    ...
    if (response.statusCode == 200) {
      ...
      for (var group in data) {
        try {
          final topicTitle = group['topic_title']?.toString() ?? 'General Questions';
...
Future<List<PastQuestion>> getRandomPastQuestions({
  ...
  try {
    ...
    final uri = Uri.parse('${ApiEndpoints.baseUrl}/api/content/past-questions/random/');
    final body = {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData is List) {
        ...
        data = responseData['results'];
      } else if (responseData is Map && responseData.containsKey('questions')) {
        data = responseData['questions'];
      } else {
        print('⚠️ Unexpected response format: ${responseData.runtimeType}');
        return [];
      ...
      final errorData = json.decode(response.body);
      final error = errorData['detail'] ?? errorData['error'] ?? 'Failed to get random questions';
      throw Exception('$error (Status: ${response.statusCode})');
    }
  } catch (e) {
...
Future<Map<String, dynamic>> getPastQuestionStats({
  ...
  try {
    ...
    final averageDifficulty = ((easyCount * 1) + (mediumCount * 2) + (hardCount * 3)) / questions.length;
...
Future<List<PastQuestion>> _getCachedPastQuestions(String cacheKey) async {
  try {
    ...
    if (cachedData != null && cachedData is List) {
      final questions = cachedData.map((json) => PastQuestion.fromJson(json)).toList();
      print('📂 Loaded ${questions.length} past questions from cache: $cacheKey');
      return questions;
    }
  } catch (e) {
...
Future<List<Map<String, dynamic>>> getTopicsForPastQuestions({int? courseId, int? outlineId}) async {
  try {
...
Future<List<StudyDocument>> getStudyGuidesForUser() async {
  try {
    ...
    if (departmentId != null && departmentId.isNotEmpty && departmentId != 'null') {
...
List<StudyDocument> _parseStudyGuidesList(List<dynamic> data) {
  ...
  for (var item in data) {
    try {
      ...
      final guide = StudyDocument(
        ...
        fileName: guideMap['file_name']?.toString() ?? 
                 _extractFileNameFromUrl(guideMap['pdf_url']?.toString()) ?? 
                 'document.pdf',
        fileUrl: guideMap['pdf_url']?.toString() ?? guideMap['pdf_file']?.toString() ?? '',
        ...
        university: guideMap['university_name']?.toString() ?? 
        ...
        semester: _parseSemester(guideMap['semester'] ?? guideMap['semester_id']),
      );
  ...
  print('📊 Successfully parsed ${guides.length} out of ${data.length} items');
...
Future<String> _getActualFileSize(String fileUrl) async {
  try {
    ...
    final response = await http.head(
      Uri.parse(fileUrl),
      headers: {'User-Agent': 'CerenixApp/1.0'},
...
String _extractFieldName(dynamic fieldData) {
  ...
  if (fieldData is Map) {
    ...
    if (fieldData['university_name'] != null) return fieldData['university_name'].toString();
...
Future<List<StudyDocument>> _getStudyGuidesByUserId(String userId) async {
  try {
    ...
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      print('✅ Successfully loaded ${data.length} guides via user ID endpoint');
      return _parseStudyGuidesList(data);
    }
...
String? _extractAcademicId(dynamic profile, Map<String, dynamic> userData, String field) {
