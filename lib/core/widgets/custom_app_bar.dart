...
import '../services/activation_status_service.dart';
...
class CustomAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  String? _getActivationGrade() {
    try {
      final activationGrade = ActivationStatusService.current.grade;
      if (activationGrade != null && activationGrade.isNotEmpty) {
        return activationGrade;
      }
      ...
      if (userData['activations'] is List &&
          (userData['activations'] as List).isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: Listenable.merge([
        ...
        ActivationStatusService.listenable,
      ]),
      builder: (context, _) {
        ...
        return AppBar(
          ...
          leading: Padding(
            ...
            child: IconButton(
              icon: Icon(Icons.segment, color: iconColor, size: 28),
          ...
          actions: [
            if (showThemeToggle) _buildThemeToggle(context, iconColor),
            if (showNotifications)
              ..._buildNotificationAction(context, iconColor),
  ...
  List<Widget> _buildNotificationAction(BuildContext context, Color iconColor) {
    return [
      Stack(
        children: [
          IconButton(
  ...
  Widget _buildThemeToggle(BuildContext context, Color iconColor) {
    return IconButton(
      tooltip: AppThemeController.instance.accessibilityLabel,
      icon: Icon(AppThemeController.instance.icon, color: iconColor),
