...
class OfflineService {
  ...
  Future<Course?> getRecentCourse() async {
    try {
      ...
      if (recent != null) {
        ...
        print(
          '✅ Loaded recent course: ${recent.code} (Downloaded: $isDownloaded)',
        );
        return recent;
      }
    } catch (e) {
  ...
  Future<void> downloadCourseForOffline({
    ...
    try {
      ...
      if (userData == null) {
        throw Exception(
          'User not logged in. Please login to download courses.',
        );
      }
      ...
      for (var i = 0; i < outlines.length; i++) {
        ...
        try {
          ...
          print(
            '⚠️ Warning: Could not fetch topics for outline ${outline.id}: $e',
          );
      ...
      final courseImageUrl = _normalizeDownloadUrl(course.imageUrl);
      ...
      final downloadedImages = <String, Map<String, String>>{};
      if (allTopics.isNotEmpty) {
        print('🖼️ Downloading topic images (if any)...');
        for (var i = 0; i < allTopics.length; i++) {
          final topic = allTopics[i];
          final imageCandidates = <String, String>{};

          final topicImage = _normalizeDownloadUrl(
            topic.displayImageUrl ?? topic.image,
          );
          if (topicImage != null && topicImage.isNotEmpty) {
            imageCandidates['topic_${topic.id}'] = topicImage;
          }

          final inlineUrls = _extractInlineImageUrls(topic);
          for (
            int imageIndex = 0;
            imageIndex < inlineUrls.length;
            imageIndex++
          ) {
            imageCandidates['topic_${topic.id}_inline_$imageIndex'] =
                inlineUrls[imageIndex];
          }

          for (final entry in imageCandidates.entries) {
            try {
              ...
              if (imagePath != null) {
                downloadedImages[entry.key] = {
                  'path': imagePath,
                  'original_url': entry.value,
                };
              }
            } catch (e) {
              print('⚠️ Could not download image ${entry.value}: $e');
            }
          }
      ...
      final offlineData = {
        ...
        'downloaded_images': {
          if (localImagePath != null)
            'course_image': {
              'path': localImagePath,
              'original_url': courseImageUrl ?? course.imageUrl ?? '',
            },
          ...downloadedImages,
        },
      ...
      final downloadedIds = offlineBox.get(
  ...
  Future<int> _estimateFileSize(
    ...
    for (final value in images.values) {
      final imagePath = value is String ? value : value['path']?.toString();
  ...
  String? _normalizeDownloadUrl(String? rawUrl) {
    ...
    return '$baseUrl/media/$rawUrl';
  }

  List<String> _extractInlineImageUrls(Topic topic) {
    final urls = <String>{};

    void collectFromText(String? content) {
      ...
      for (final match in htmlMatches) {
        final normalized = _normalizeDownloadUrl(match.group(2));
        if (normalized != null) {
          urls.add(normalized);
        }
      }

      final markdownMatches = RegExp(
        r'!\[[^\]]*\]\(([^)]+)\)',
      ).allMatches(content);
      for (final match in markdownMatches) {
        final normalized = _normalizeDownloadUrl(match.group(1));
        if (normalized != null) {
          urls.add(normalized);
        }
      }
    ...
    collectFromText(topic.content);
    collectFromText(topic.completionQuestionText);
    collectFromText(topic.solutionText);
  ...
  Future<Map<String, dynamic>> getDownloadStatus(String courseId) async {
    try {
      ...
      return {'status': null, 'progress': 0.0, 'isDownloading': false};
    }
  }
  ...
  Future<List<Topic>> getOfflineTopicsForOutline(
    String courseId,
    String outlineId,
  ) async {
    try {
