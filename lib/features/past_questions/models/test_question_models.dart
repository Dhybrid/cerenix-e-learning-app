  ...
  factory TestQuestionTopic.fromJson(Map<String, dynamic> json) {
    ...
    try {
      ...
      return TestQuestionTopic(
        ...
        outlineId:
            outlineInfo?['id']?.toString() ?? json['outline']?.toString(),
        courseId:
...
class TestQuestion {
  ...
  factory TestQuestion.fromJson(Map<String, dynamic> json) {
    ...
    try {
      return TestQuestion(
        ...
        sessionId:
            json['session_id']?.toString() ?? json['session']?.toString(),
  ...
  // New helper method to check if this is an image-only question
  bool get isImageOnlyQuestion {
    // Check if question_text is null, empty, or only contains empty HTML tags
    final hasNoText =
        questionText == null ||
        questionText!.trim().isEmpty ||
        questionText == '<p></p>' ||
        questionText == '<p><br></p>' ||
        questionText == '<p>\u200B</p>' || // Zero-width space
        questionText!.replaceAll(RegExp(r'<[^>]*>|&[^;]+;'), '').trim().isEmpty;

    // Check if it has a question image
    final hasImage = questionImageUrl != null && questionImageUrl!.isNotEmpty;
