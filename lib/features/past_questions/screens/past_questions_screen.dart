...
class _PastQuestionsScreenState extends State<PastQuestionsScreen> {
  ...
  //     // FIRST: ALWAYS try to load from offline storage first
  //     print('📴 Checking for offline questions first...');
  ...
  //       // Don't try online if we have offline data
  //       return;
  //     }

  //     // SECOND: Only try online if we have no offline data
  //     print('📡 No offline questions found, checking connectivity...');
  ...
  //       // Quick network test to confirm
  //       if (isOnline) {
  //         // Try a small network request to confirm
  ...
  //     if (!isOnline) {
  //       print('📴 No connectivity and no offline data available');
  //       setState(() {
  //         _isOffline = true;
  //         _errorMessage = 'You are offline and no downloaded questions found.';
  //         _isLoading = false;
  //       });
  //       return;
  //     }
  ...
  //       // Cache questions for future offline use
  //       await _cacheQuestionsForOffline(apiQuestions);
  //     } catch (apiError) {
  //       print('❌ API call failed: $apiError');

  //       // Fall back to any offline data we might have
  //       if (offlineQuestions.isEmpty) {
  //         setState(() {
  //           _isOffline = true;
  //           _errorMessage = 'Failed to load questions. You may be offline.';
  //           _isLoading = false;
  //         });
  //       }
  //     }
  ...
  //       // Check sessionInfo map first (most reliable)
  //       if (question.sessionInfo.isNotEmpty) {
  //         questionSessionId = question.sessionInfo['id']?.toString();
  //         print(
  //           '   Question ${question.id} sessionInfo ID: $questionSessionId',
  //         );
  //       }

  //       // Fallback to sessionId field
  //       if ((questionSessionId == null || questionSessionId.isEmpty) &&
  //           question.sessionId != null) {
  //         questionSessionId = question.sessionId;
  //         print(
  //           '   Question ${question.id} sessionId field: $questionSessionId',
  //         );
  //       }

  //       // If we still don't have a session ID, check if it's embedded in other fields
  //       if (questionSessionId == null || questionSessionId.isEmpty) {
  //         // Try to extract from session_info field
  //         if (question.sessionInfo.containsKey('session_info')) {
  //           final nestedSessionInfo = question.sessionInfo['session_info'];
  //           if (nestedSessionInfo is Map) {
  //             questionSessionId = nestedSessionInfo['id']?.toString();
  //             print(
  //               '   Question ${question.id} nested session_info ID: $questionSessionId',
  //             );
  //           }
  //         }
  //       }

  //       // If question has no session info at all, decide what to do
  //       if (questionSessionId == null || questionSessionId.isEmpty) {
  //         print('   ⚠️ Question ${question.id} has no session ID');

  //         // In offline mode, we might want to include questions without session info
  //         // This handles legacy/downloaded questions without proper session info
  //         if (widget.sessionId == null || widget.sessionId!.isEmpty) {
  //           return true; // Include if no specific session filter
  //         }
  //         return false; // Exclude if specific session filter but no session ID
  //       }

  //       // Compare session IDs
  //       final matches = questionSessionId == widget.sessionId;
  //       print(
  //         '   Question ${question.id} session match: $matches ($questionSessionId vs ${widget.sessionId})',
  //       );
  //       return matches;
  //     }).toList();
  //   }
  ...
  //       if (question.topicInfo != null && question.topicInfo!['id'] != null) {
  //         questionTopicId = question.topicInfo!['id']?.toString();
  //       }

  //       // Check topicId field
  //       if (questionTopicId == null && question.topicId != null) {
  ...
  //       if (!matches) {
  //         print(
  //           '   Question ${question.id} topic mismatch: $questionTopicId != ${widget.topicId}',
  //         );
  //       }
  //       return matches;
  //     }).toList();
